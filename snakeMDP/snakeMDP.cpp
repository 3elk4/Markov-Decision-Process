...
snakeMDP::snakeMDP(QWidget *parent) : QMainWindow(parent), ui(new Ui::snakeMDPClass)
{
	ui->setupUi(this);
	...
	PolicyIteration policyIteration = PolicyIteration<pair<Point, Point>, action>(mdp);
...
void snakeMDP::paintEvent(QPaintEvent * event)
{
	QPainter paint(this);
...
Point snakeMDP::getRandomFoodPos()
{
	//TODO: copy or not?
...
void snakeMDP::loop() {
	...
	step_details sd = mdp.step(this->policy[this->snakeAndFood], this->snakeAndFood); 
